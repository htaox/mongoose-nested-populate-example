...
Populator.prototype.run = function() {
  ...
  async.eachSeries(this.map, function(m, next) {
    try {
      ...
      var refsOnly = _.reduce(m.members, function(memo, d) {
        if (d.model) {
          memo.push(d);
        }
        return memo;
      ...
      if (refsOnly.length > 0) {
        self.models[m.name].populate(self.data, refsOnly, function(err, d) {
          next(null, d);
        });
      } else {
        next(null, self.data);
    ...
    if (self.debug) {
      require('fs').writeFileSync(__dirname + '/DEBUG/' + self.data.id + '.json', JSON.stringify(
        self.data, null, '\t'), {
        encoding: 'utf8'
      });
    }
...
Inhabitant.prototype.explore = function(model, parent) {
  ...
  var treed = _.map(tree, function(v, k) {
    var currentPath = (parent ? parent + '.' : '') + k;
    ...
    if (self.config.exploreAll && v.type && !v.ref && Object.prototype.toString.call(v) != '[object Array]') {
      return {
        shortPath: k,
        path: currentPath,
        type: v.type.name.toLowerCase()
      }
    }
  ...
  */
  ...
  model.find(filter, null, options, function(err, arr) {
    ...
    async.mapSeries(arr, function(d, cb) {
      ...
      if (self.debug)
        console.log('model: ' + modelName + '|count: ' + results.length + '|elapsed: ' +
          process.hrtime(time));
